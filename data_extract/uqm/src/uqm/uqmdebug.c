...
calculateMineralValue (const SOLARSYS_STATE *system, const PLANET_DESC *world)
{
	...
	for (i = 0; i < numDeposits; i++)
	{
		...
		// stuart_new_code
		elem_count = HIBYTE (system->SysInfo.PlanetInfo.CurDensity);
		elem_id = GLOBAL (ElementWorth[ElementCategory (
				system->SysInfo.PlanetInfo.CurType)]);
		...
		if (elem_id == 25){
			...
			accum[elem_id - 1] += elem_count;
		}
		//printf("This is a test - digit = %d (%d)\n", elem_count, elem_id);
	}

	// Print out array of element amounts for each type
	printf("minz_s|");
	for (i = 0; i < 8; i++)
	{
		printf("%d|", accum[i]);
